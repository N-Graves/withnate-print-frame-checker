...
const JS = new URL("../dist/print-frame-checker.js", import.meta.url);
const CSS = new URL("../dist/print-frame-checker.css", import.meta.url);
...
const foreign = [...new Set(selectors.map((s) => s.trim()))].filter((s) => !s.startsWith(".pfc"));
check(foreign.length === 0, "defines only .pfc- classes", foreign.join(" "));
